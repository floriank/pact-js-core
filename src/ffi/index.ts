...
const initialiseFfi = (logLevel: LogLevel,logFile?:string): typeof ffi => {
  ...
  if (logFile) {
    ...
    logger.debug(`Setting log file to '${convertLogLevelToFfiLoglevelFilter[logLevel]}'`);
    const res = ffiLib.pactffiLogToFile(logFile,convertLogLevelToFfiLoglevelFilter[logLevel]);
